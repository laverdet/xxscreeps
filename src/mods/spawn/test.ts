...
describe('isActive', () => {
	...
	test('excess spawns at RCL 1 — one inactive', () => excessSpawnSim(async ({ player }) => {
		await player('100', Game => {
			...
			const inactiveCount = spawns.filter(s => !s.isActive()).length;
			assert.strictEqual(activeCount, 1, 'exactly 1 spawn should be active at RCL 1');
			assert.strictEqual(inactiveCount, 1, 'exactly 1 spawn should be inactive at RCL 1');
		});
	}));
	...
	test('inactive spawn returns ERR_RCL_NOT_ENOUGH', () => excessSpawnSim(async ({ player }) => {
		await player('100', Game => {
			const spawns = lookForStructures(Game.rooms.W3N1, C.STRUCTURE_SPAWN);
			const inactive = spawns.find(s => !s.isActive())!;
	...
	test('energyAvailable excludes inactive extensions', () => energySim(async ({ player }) => {
		await player('100', Game => {
			...
			// energyAvailable should only include spawn energy, not the inactive extension
	...
	const ownerMismatchSim = simulate({
		W4N1: room => {
			room['#insertObject'](create(new RoomPosition(25, 25, 'W4N1'), '100', 'ConqueredSpawn'));
			room['#level'] = 4;
			room['#user'] =
	...
	const distanceSim = simulate({
		W5N1: room => {
			...
			room['#insertObject'](create(new RoomPosition(controllerPos.x - 1, controllerPos.y, 'W5N1'), '100', 'CloseSpawn'));
			// Spawn far from controller
			room['#insertObject'](create(new RoomPosition(1, 1, 'W5N1'), '100', 'FarSpawn'));
