...
import { create as createPowerSpawn } from 'xxscreeps/mods/powerspawn/powerspawn.js';
...
describe('PowerCreep spawned', () => {
	...
	// The harness never materializes the account roster into the runtime, so the unspawned creep is
	// constructed inline and spawned through the real intent path; once processed it is a normal room
	// object reachable through `Game.powerCreeps`.
	test('spawn places the creep next to the power spawn with a full lifetime', () => sim(async ({ player, tick }) => {
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			const alice = createPowerCreep(Id.generateId(), 'Alice', C.POWER_CLASS.OPERATOR, owner);
			assert.strictEqual(alice.spawn(powerSpawn), C.OK);
		});
		await tick();
		await player(owner, Game => {
			const alice = Game.powerCreeps.Alice!;
			assert.strictEqual(alice.room.name, 'W1N1');
			assert.strictEqual(alice.pos.isNearTo(spawnPos), true);
			assert.strictEqual(alice.ticksToLive, C.POWER_CREEP_LIFE_TIME);
			assert.strictEqual(alice.hits, 1000);
		});
	}));

	test('spawn on an already-spawned creep returns ERR_BUSY', () => sim(async ({ player, tick }) => {
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			assert.strictEqual(createPowerCreep(Id.generateId(), 'Alice', C.POWER_CLASS.OPERATOR, owner).spawn(powerSpawn), C.OK);
		});
		await tick();
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
	...
	test('a spawned power creep moves without fatigue', () => sim(async ({ player, tick }) => {
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			assert.strictEqual(createPowerCreep(Id.generateId(), 'Alice', C.POWER_CLASS.OPERATOR, owner).spawn(powerSpawn), C.OK);
		});
		...
		await player(owner, Game => {
			const alice = Game.powerCreeps.Alice!;
			// Step one more square away from the spawn, into open terrain.
			const direction = spawnPos.getDirectionTo(alice.pos);
			target = getPositionInDirection(alice.pos, direction)!;
			assert.strictEqual(alice.move(direction), C.OK);
		});
		await tick();
		await player(owner, Game => {
	...
	test('a spawned power creep drops a carried resource', () => sim(async ({ player, poke, tick }) => {
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			assert.strictEqual(createPowerCreep(Id.generateId(), 'Alice', C.POWER_CLASS.OPERATOR, owner).spawn(powerSpawn), C.OK);
		});
	...
	test('suicide leaves a tombstone and starts the respawn cooldown', () => sim(async ({ player, peekRoom, tick, shard }) => {
		...
		const [ created ] = await Model.loadRoster(shard.db, owner);
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			assert.strictEqual(createPowerCreep(created!.id, 'Alice', C.POWER_CLASS.OPERATOR, owner).spawn(powerSpawn), C.OK);
		});
		await tick();
		await player(owner, Game => {
	...
	test('spawn is rejected while a roster spawn cooldown is still pending', () => sim(async ({ player, peekRoom, tick, shard }) => {
		...
		await Model.setSpawnCooldown(shard.db, owner, created!.id, Date.now() + C.POWER_CREEP_SPAWN_COOLDOWN);
		await player(owner, Game => {
			const powerSpawn = lookForStructures(Game.rooms.W1N1, C.STRUCTURE_POWER_SPAWN)[0]!;
			// The inline runtime creep carries a stale cooldown of 0, so the runtime check passes; the
			// processor must reject it against the authoritative roster.
			assert.strictEqual(createPowerCreep(created!.id, 'Alice', C.POWER_CLASS.OPERATOR, owner).spawn(powerSpawn), C.OK);
		});
		await tick();
		await peekRoom('W1N1', room => {
	...
	test('lethal damage routes death through the tombstone + respawn cooldown', () => sim(async ({ player, poke, peekRoom, tick, shard }) => {
		...
		await tick();
		// Combat reaches this through `#applyDamage` → tick settlement; drive hits to zero directly.
		await poke('W1N1', owner, (Game, room) => {
