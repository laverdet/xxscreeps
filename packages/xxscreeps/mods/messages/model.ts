 ...
 */
export async function sendMessage(db: Database, senderId: string, respondentId: string, text: string): Promise<Message> {
 ...
 */
export async function getConversationIndex(db: Database, userId: string): Promise<ConversationIndex> {
 ...
 */
export async function getConversation(db: Database, userId: string, respondentId: string, limit = 100): Promise<Message[]> {
 ...
 */
export async function markRead(db: Database, userId: string, messageId: string): Promise<boolean> {
...
/**
 * Remove all of a user's message state. Drops their threads, conversation index and unread set, and
 * the message documents they own. The peer (other party) keeps their own copies; orphaned references
 * are tolerated by the read paths.
 */
