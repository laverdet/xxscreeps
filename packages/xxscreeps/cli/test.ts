...
describe('cli', () => {
	describe('repl', () => {
		test('vars persist across turns', async () => {
			try {
		...
		test('statement with top-level await falls through to async-block', async () => {
		...
		test('top-level await whose awaited callback throws SyntaxError runs source once', async () => {
		...
		test('parenthesised top-level await falls through to async wrap', async () => {
		...
		test('SyntaxError unrelated to top-level await still rejects', async () => {
		...
		test('incomplete input is reported recoverable', () => {
		...
		test('meta-commands work in a real session', async () => {
	...
	describe('eval', () => {
		test('runtime SyntaxError does not retrigger console.log', async () => {
		...
		test('-e prints inspected result with exit 0', async () => {
			using console = new TestConsole();
			const exitCode = await runEval({
				argv: [],
		...
		test('thrown error writes to stderr with exit 1', async () => {
			using console = new TestConsole();
			const exitCode = await runEval({
				argv: [],
		...
		test('argv is exposed to the script', async () => {
			using console = new TestConsole();
			const exitCode = await runEval({
				argv: [ 'one', 'two' ],
				source: 'argv.join("|")',
		...
		test('--file reads source from the filesystem', async () => {
		...
		test('--stdin reads source from stdin', async () => {
