...
- 87fdd36: Effects and renderers compose across mods instead of replacing what another mod bound.
...
- 92da93e: Decorations are opt-in: `decorations.grantAll` and the new `decorations.inventory` both default off, so a server renders what is placed without handing anybody the catalog or the client's inventory section. Taking a decoration down stays served either way.
...
- 1183326: `xxscreeps export` reports the objects no codec claims, counted by class, on stderr.
...
- 4795a33: Bundled world and bot data replace the `@screeps/launcher` dependency.
...
- 9c40aab: Notification transports split into mods: keyval persistence in meta/notify-cron, raw stdout opt-in.
...
- 581bccf: Payloads now carry keeper lairs and the prebuilt neutral extractor.
...
- b87582d: Loot the ruin of a damage-destroyed invader core; stronghold ruins last until collapse.
